...
pub struct ImageTransform {
    ...
    pub dpi: Option<f32>,
    /// Optional clipping rectangle in page coordinates (Mm, bottom-left origin).
    /// When set, the image is clipped to this rectangle.
    pub clip_rect: Option<[Mm; 4]>, // [ll_x, ll_y, ur_x, ur_y]
}
...
impl Image {
    ...
    pub fn add_to_layer(self, layer: PdfLayerReference, transform: ImageTransform) {
        ...
        // If a clip rect is specified, wrap the image in q/clip/Q.
        // The clip path is emitted INSIDE the graphics-state save so it is
        // scoped to this image only, and BEFORE the transform so it is in
        // page-space coordinates.
        if let Some([ll_x, ll_y, ur_x, ur_y]) = transform.clip_rect {
            use lopdf::content::Operation;
            use lopdf::Object::Real;

            let to_pt = |mm: Mm| mm.0 * 72.0 / 25.4;
            let x = to_pt(ll_x);
            let y = to_pt(ll_y);
            let w = to_pt(ur_x) - x;
            let h = to_pt(ur_y) - y;
