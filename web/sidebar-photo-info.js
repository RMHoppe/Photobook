...
export class SidebarPhotoInfoPanel {
    ...
    _currentSingleId = null;
    constructor(containerEl, sidebar) {
    ...
    _showSingle(id) {
        ...
        if (dims) {
            ...
            this._sidebar.ensureDimensions(id).then(resolved => {
                if (!resolved || this._currentSingleId !== id)
                    return;
        ...
        if (fileSize !== null)
            rows.push(['Size', formatBytes(fileSize)]);
        meta.innerHTML = rows.map(([k, v]) => `<div class="pi-meta-row">` +
