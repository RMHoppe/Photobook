...
export class InlineEditor {
    ...
    _ox = 0; // left edge of bounding box
    _hw = 0; // initial half-width (for centre/right anchor calculation)
    _xMm = 0; // el.x_mm at edit start (mm) — for x_mm correction on resize
    _dirty = false;
    editor;
    renderer;
    cb;
    constructor(areaEl, editor, renderer, callbacks) {
        this.editor = editor;
    ...
    get isActive() { return this.renderer.editingTextId !== null; }
    // ---------------------------------------------------------------------------
    // Public API
    // ---------------------------------------------------------------------------
    start(textId) {
    ...
    stop() {
        if (this.renderer.editingTextId === null)
            return;
