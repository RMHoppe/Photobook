...
export class SplitterEditor {
    ...
    _built = false;
    _emitTimer = null;
    constructor(containerEl, onBoxModelChange, onRatioChange) {
    ...
    update(bm, ratio, axis) {
        if (!this._built || this.containerEl.dataset.panel !== 'splitter')
            this._build();
        const active = document.activeElement;
        const setNum = (field, v) => {
            const el = this.containerEl.querySelector(`[data-field="${field}"]`);
            if (!el || active === el)
                return;
            if (v < 0) {
                el.value = '';
        ...
        const ratioEl = this.containerEl.querySelector('[data-field="sp-ratio"]');
        if (ratioEl && active !== ratioEl) {
            if (ratio < 0) {
                ratioEl.value = '';
                ratioEl.placeholder = 'Mixed';
            }
            else {
                ratioEl.value = (ratio * 100).toFixed(1);
                ratioEl.placeholder = '';
            }
        }
        const axisEl = this.containerEl.querySelector('.sp-axis-label');
        if (axisEl) {
        ...
        const rotEl = this.containerEl.querySelector('[data-field="node-rotation"]');
        if (rotEl && active !== rotEl) {
            const rot = bm.node_rotation_deg ?? 0;
            if (Math.abs(rot) >= 9998) {
                rotEl.value = '';
                rotEl.placeholder = 'Mixed';
            }
            else {
                rotEl.value = rot.toFixed(2);
                rotEl.placeholder = '';
            }
        }
    }
    ...
    _numField(name, label, min = 0, max = 200, step = 0.5) {
    ...
    _scheduleRatio(el) {
        ...
        this._emitTimer = setTimeout(() => {
            ...
            if (!isNaN(v))
                this.onRatioChange(Math.max(5, Math.min(95, v)) / 100);
        }, 150);
    }
    _scheduleBoxModel() {
        if (this._emitTimer !== null)
        ...
        this._emitTimer = setTimeout(() => {
            ...
            const gSel = (field) => {
                const el = this.containerEl.querySelector(`[data-field="${field}"]`);
                return (!el || el.dataset.mixed) ? '' : el.value;
            };
            const rotEl = this.containerEl.querySelector('[data-field="node-rotation"]');
