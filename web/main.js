...
const overlays = { marqueeRect: null, splitPreview: null, swapOverlay: null, edgeDragPreview: null, imageDropPreview: null };
function fitCanvas() {
...
// Image sidebar
// ---------------------------------------------------------------------------
const sidebar = new ImageSidebar(document.getElementById('image-grid'), document.getElementById('folder-breadcrumb'), 
...
// Image-loader modal
// ---------------------------------------------------------------------------
const imageLoaderModal = new ImageLoaderModal(async (id, buf) => {
...
function showMissingImagesBanner(missingIds) {
...
async function checkMissingFonts() {
    fontToast.hidden = true;
...
function wireRightSidebar() {
    ...
    const boxEditor = new BoxModelEditor(panelFace, editorCallback((json) => {
        undoManager.snapshot();
        ...
        switch (transform) {
            case 'flip-h':
                editor.flip_selection_h();
                break;
            case 'flip-v':
                editor.flip_selection_v();
                break;
            case 'rotate-cw':
    ...
    projectPanel.setEndpapersToggleHandler((enabled) => {
        undoManager.snapshot();
...
function refreshBoxModel() {
    ...
    if (hasDivider)
        dividerPanel.show(editor.get_selected_segment_gap());
    if (showPhoto)
...
// Footer
// ---------------------------------------------------------------------------
const footer = new Footer(document.getElementById('spread-thumbnails'), document.getElementById('btn-prev-spread'), document.getElementById('btn-next-spread'), document.getElementById('btn-add-spread'), document.getElementById('btn-remove-spread'), document.getElementById('spread-count-label'), (idx) => {
...
function showFontAccessWarning(code) {
    const msg = fontAccessWarningDialog.querySelector('#font-warning-msg');
...
function showCannotDeleteReason() {
    const min = editor.get_endpapers() ? 3 : 2;
    const msg = cannotDeleteDialog.querySelector('#cannot-delete-msg');
...
// Inline text editor
// ---------------------------------------------------------------------------
const inlineEditor = new InlineEditor(document.getElementById('canvas-center'), editor, renderer, {
...
canvasEl.addEventListener('dragover', (e) => {
    ...
    if (hitId === NULL_ID) {
        if (overlays.imageDropPreview !== null) {
    ...
    if (imageIds.length === 1) {
        ...
        if (zone !== 'center') {
            ...
            if (zone === 'left' || zone === 'top') {
                if (existingImageId)
                    editor.assign_image(faceIds[1], existingImageId);
...
{
    const dlg = document.getElementById('save-name-dialog');
    const input = document.getElementById('save-name-input');
    const btnOk = document.getElementById('btn-save-name-ok');
    const btnCan = document.getElementById('btn-save-name-cancel');
...
textEditor.setLoadFontsHandler(async () => {
    ...
    if (result.families.length > 0)
        textEditor.setFontFamilies(result.families);
});
